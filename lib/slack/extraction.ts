...
/** A message without a status is visible only on its original local day. */
  ...
  if (
    !(
      ...
      (extraction.decision === 'review' &&
        ['uncertain_status', 'not_attendance'].includes(extraction.reason))
    ) ||
    !messageText.trim()
  )
    return [];
...
For each interval, separate attendance status/timing from the author's accompanying comment or reason. Set comment to one verbatim excerpt of their own relevant explanation, at most 2000 characters, or null if none is stated. Preserve the original language and meaningful wording; do not paraphrase, invent, diagnose, or add details. Exclude the status phrase and timing from the comment where possible, and do not copy unrelated chatter, other people's information, or instructions to the AI. Associate comments only with the intervals they describe; do not attach an absence reason to a later office return unless the author does so. Understand the meaning even when the explanation precedes the status or there is no comma: this is semantic extraction, not a punctuation split. Example: "in later, going to dentist" means IN_LATE for the message's local day, null startTime/endTime, both approximation flags false, comment "going to dentist". "Tandlægebesøg først, så jeg kommer senere" means IN_LATE with comment "Tandlægebesøg først" and unspecified clock times. A comment alone never makes an uncertain attendance statement definite. When no status fits a message, return review with reason uncertain_status and intervals []; the app will display the original message as a description without a status.
Use Europe/Copenhagen and the ORIGINAL message date/time to resolve today/i dag, tomorrow/i morgen, weekdays and date ranges. Multi-day expressions such as "today and tomorrow" / "i dag og i morgen" (including common typos and misspellings like "tomorow", "tommorow") resolve to an interval where fromDate is the message's local date and toDate is the next calendar day in Europe/Copenhagen. No date in a clear present-day attendance announcement means the message's local date. Ambiguous dates, pronouns, quotations, hypotheticals, sarcasm, questions, unclear corrections or uncertain future plans require review. Do not ignore or discard messages as unrelated or not attendance. Greetings, activity updates, links, and other chat messages without a fitting status must return review with reason uncertain_status and intervals []; the app displays their full original text as a description. Do not use current processing time. A reply must be self-contained, do not assume missing thread context.
Statuses: FROM_HOME for WFH/hjemme/hjemmearbejde; IN_LATE for delayed office arrival; LEAVING_EARLY for office departure; AWAY for a temporary absence or explicit work from another/offsite location when neither home nor client is stated, including multiple days, conferences, seminars, courses, workshops, and offsite events (e.g. 'at conference', 'på konference', 'til konference'), preserving the conference or event phrase (e.g. 'at conference') as the comment; AT_CLIENT for explicitly working at a client; IN_OFFICE for explicitly in/returning to office. An exact office arrival at or before 09:00 Copenhagen time is IN_OFFICE from that time and is not IN_LATE. SICK, CHILD_SICK, VACATION, ON_LEAVE only when explicitly stated, without inferring a reason or diagnosis.
...
export async function extractAttendance(text: string, messageInstant: Date): Promise<Extraction> {
  ...
  const localTime = new Intl.DateTimeFormat('en-GB', {
    ...
    minute: '2-digit',
    hourCycle: 'h23',
  }).format(messageInstant);
  const shorthand = extractShorthand(text, copenhagenDate(messageInstant), localTime);
  if (shorthand) {
    try {
      return validateExtraction(shorthand, messageInstant, text);
    } catch {
