...
export function ProfileListItem({ user, showStatus = false }: ProfileListItemProps) {
  ...
  const actionBound = imported
    ? user.status?.status === 'IN_LATE' && user.status.details !== 'Arrival time unspecified'
      ? user.status.endsAt
      : user.status?.status === 'LEAVING_EARLY' &&
          user.status.details !== 'Departure time unspecified'
  ...
  const formattedInterval =
    imported && user.status?.status !== 'IN_LATE' && user.status?.status !== 'LEAVING_EARLY'
      ? formatInterval(
          user.status?.startsAt,
          user.status?.endsAt,
